// ==== Badge sisa kredit (di bawah tombol hamburger) + modal "Tingkatkan ke Pro" ====
function renderCreditBadge() {
  const el = document.getElementById('credit-badge');
  const numEl = document.getElementById('credit-badge-num');
  if (!el || !numEl) return;
  if (!(typeof sbSession !== 'undefined' && sbSession && sbProfile)) { el.style.display = 'none'; return; }
  const total = sbCreditTotal();
  el.style.display = '';
  numEl.textContent = 'Credit ' + (total >= 1000 ? (total / 1000).toFixed(1).replace(/\.0$/, '') + 'K' : String(total));
  el.title = total + ' kredit tersisa';
  el.classList.toggle('low', total <= 20);
}
function openUpgradeModal() {
  document.getElementById('upgrade-modal-backdrop').classList.add('show');
  document.getElementById('upgrade-modal').classList.add('show');
}
// Diklik dari badge kredit — nampilin info apa adanya, BUKAN selalu nganggep kredit abis.
// Modal "Kredit Abis" cuma boleh muncul kalau emang beneran abis (dicek dari deduct_credit RPC).
function clickCreditBadge() {
  if (!sbProfile) return;
  const total = sbCreditTotal();
  if (total <= 0) { openUpgradeModal(); return; }
  toast('Sisa kredit: ' + sbProfile.credit_awal_sisa + ' (awal) + ' + sbProfile.credit_harian_sisa + ' (harian) = ' + total);
}
function closeUpgradeModal() {
  document.getElementById('upgrade-modal-backdrop').classList.remove('show');
  document.getElementById('upgrade-modal').classList.remove('show');
}
function clickUpgradeToPro() {
  closeUpgradeModal();
  openRedeemModal(); // upgrade beneran (auto) masih nunggu payment gateway; buat sekarang lewat redeem code
}
// Dipanggil abis AI kelar jawab (atau berhenti manual tapi masih ada teks).
// Prompt user: 1 huruf = 1 kredit. Balasan AI: 1 huruf = 3 kredit. Dipotong sekali gabungan.
async function chargeCreditForExchange(userText, aiText) {
  if (!(typeof sbSession !== 'undefined' && sbSession)) return;
  const total = (userText ? userText.length : 0) * 1 + (aiText ? aiText.length : 0) * 3;
  if (total <= 0) return;
  const result = await sbDeductCredit(total);
  renderCreditBadge();
  if (result && result.allowed === false) openUpgradeModal();
}
...
// Dipanggil langsung dari tombol "Select Plan" di tiap kartu — milih sekaligus konfirmasi
// dalam satu klik (gak perlu tombol "Lanjutkan" terpisah lagi).
function chooseAndConfirmPlan(p) {
  selectPlanBox(p);
  confirmPlanSelection();
}
function confirmPlanSelection() {
  if (!pendingPlanChoice || pendingPlanChoice === 'pro' || pendingPlanChoice === 'maks' || pendingPlanChoice === 'promax') return;
  ...
  if (sbSession) sbSetUserPlan(pendingPlanChoice).then(() => renderCreditBadge());
...
async function submitRedeemCode() {
  ...
  try {
    ...
    if (data.type === 'plan' && data.plan && PLANS[data.plan]) {
      ...
      applyPlan(data.plan);
      localStorage.setItem(scopedKey(LS_PLAN_CHOSEN), '1');
      if (sbSession) sbSetUserPlan(data.plan).then(() => { renderCreditBadge(); closeUpgradeModal(); });
...
function renderSidebarAccount() {
  ...
  if (subEl) {
    let sub = 'Akun lokal';
    if (typeof sbSession !== 'undefined' && sbSession) sub = 'Login via Google';
    else if (localStorage.getItem(LS_IS_TRIAL) === '1') sub = 'Akun Uji Coba';
    else if (userEmail) sub = 'Login via Email';
...
function confirmLogout() {
  ...
  if (wasSupabase) {
    sbSignOut(); // ini bakal trigger event SIGNED_OUT -> handleSupabaseSignOut() di 03-ui-interactions.js
  } else {
