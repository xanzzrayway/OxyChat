...
function modelCardHTML(value, small) {
  ...
  return '<div class="model-card' + (small ? ' model-card-sm' : '') + (sel ? ' selected' : '') + (locked ? ' locked' : '') + '" onclick="selectModelFromSheet(\'' + value + '\')">'
    ...
    + (locked ? '<span class="model-card-pro-badge">PRO</span>' : '')
...
function showWelcomeOverlay() {
  ...
  const remembered = localStorage.getItem(LS_REMEMBERED_EMAIL);
  const emailInput = document.getElementById('welcome-email-input');
  const rememberCheck = document.getElementById('welcome-remember-check');
  if (remembered && emailInput && rememberCheck) {
    emailInput.value = remembered;
    rememberCheck.checked = true;
  }
}
function toggleWelcomePwd() {
  const input = document.getElementById('welcome-password-input');
  const eye = document.getElementById('welcome-eye-icon');
  if (!input || !eye) return;
  const show = input.type === 'password';
  input.type = show ? 'text' : 'password';
  eye.innerHTML = show
    ? '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M2 2l20 20M9.9 9.9a3 3 0 0 0 4.2 4.2M6.5 6.7C4 8.3 2 12 2 12s4 7 11 7c1.9 0 3.6-.5 5-1.3M12 5c7 0 11 7 11 7a19 19 0 0 1-2.2 3"/></svg>'
    : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7z"/><circle cx="12" cy="12" r="3"/></svg>';
}
// Login via email & password: belum ada backend yang cek password beneran (server cuma nyimpen
// API key, bukan akun+password), jadi ini bikin sesi lokal pake email yang diketik — sama kayak
// alur trial, tapi identitasnya beneran dari input, bukan "Trial Account". Login Google di atas
// tetap yang asli/terverifikasi via OAuth.
function handleEmailLogin(event) {
  event.preventDefault();
  const email = document.getElementById('welcome-email-input').value.trim();
  const remember = document.getElementById('welcome-remember-check').checked;
  if (!email) return;
  if (remember) localStorage.setItem(LS_REMEMBERED_EMAIL, email);
  else localStorage.removeItem(LS_REMEMBERED_EMAIL);
  userName = email.split('@')[0] || 'Pengguna';
  userPicture = '';
  userEmail = email;
...
function triggerGoogleLogin() {
  sbSignInWithGoogle().catch(() => toast('Login Google gagal dimulai, coba lagi ya'));
}
// Dipanggil dari listener auth Supabase (00-supabase.js) begitu sesi Google berhasil terdeteksi
// (baik abis redirect OAuth, maupun sesi lama yang masih valid pas app dibuka lagi).
// PENTING: apapun yang error di dalam sini, overlay login HARUS tetep ketutup di akhir —
// sebelumnya kalau sbFetchProfile() gagal (server lambat/RLS/dst), fungsi ini `return` duluan
// sebelum sempet nutup overlay, jadi user keliatan "gak pernah bisa login" padahal Google-nya
// udah beneran berhasil. Sekarang dibungkus try/finally + fallback data biar gak pernah nyangkut.
let _completingLogin = false;
async function completeSupabaseLogin() {
  if (_completingLogin) return; // cegah kepanggil dobel bareng (INITIAL_SESSION + SIGNED_IN)
  _completingLogin = true;
  try {
    await sbFetchProfile();
    const googleName = (sbSession.user.user_metadata && (sbSession.user.user_metadata.full_name || sbSession.user.user_metadata.name)) || '';
    const googlePicture = (sbSession.user.user_metadata && (sbSession.user.user_metadata.avatar_url || sbSession.user.user_metadata.picture)) || '';
    if (!sbProfile) {
      // Fallback: profil di server gagal/belum ke-fetch — tetep loloskan masuk pake data dari
      // akun Google-nya langsung, jangan sampe user nyangkut di layar login gara-gara ini doang.
      toast('Sinkron akun belum sempurna, coba lagi nanti (chat tetep bisa dipake)');
      sbProfile = { name: googleName, picture: googlePicture, email: sbSession.user.email || '', plan: 'gratis', plan_chosen: true, credit_awal_sisa: 0, credit_harian_sisa: 0 };
    }
    userName = sbProfile.name || googleName || (sbSession.user.email || '').split('@')[0] || 'Pengguna';
    userPicture = sbProfile.picture || googlePicture || '';
    userEmail = sbProfile.email || sbSession.user.email || '';
    localStorage.setItem(LS_NAME, userName);
    ...
    migrateLegacyAccountData(accountKey());
    reloadAccountScopedState();
    currentPlan = sbProfile.plan || 'gratis';
    localStorage.setItem(scopedKey(LS_PLAN), currentPlan);

    try { await sbRefreshDailyCredit(); } catch (e) {}
    try {
      const remote = await sbLoadAllConversations();
      if (remote && Object.keys(remote).length) {
        sessions = Object.assign({}, sessions, remote);
    ...
    renderSidebarAccount();
    renderHistList();
    if (activeId && sessions[activeId]) { renderMessages(); updateTopbarTitle(); } else { showEmptyState(); }
    if (typeof renderCreditBadge === 'function') renderCreditBadge();
    buildModelDD();
    toast('Halo, ' + userName + '!');
    if (!sbProfile.plan_chosen) showPlanOverlay();
  } catch (err) {
    toast('Ada masalah pas login, tapi chat tetep bisa dipake');
  } finally {
    hideWelcomeOverlay(); // ini WAJIB jalan apapun yang terjadi di atas
    _completingLogin = false;
  }
}
// Dipanggil kalau ternyata gak ada sesi Supabase tersimpan (device baru / abis logout) —
// login lokal (trial/email) tetep dicek dulu sebelum mutusin nampilin layar login. Tapi kalau
// terakhir login-nya emang lewat Google (Supabase), berarti sesinya abis/gak valid lagi —
// jangan biarin nyangkut keliatan "masih login" padahal request ke Supabase bakal ke-tolak semua.
function onSupabaseNoSession() {
  const wasGoogleUser = !!localStorage.getItem(LS_GOOGLE_NAME);
  if (wasGoogleUser) {
...
function handleSupabaseSignOut() {
  userName = ''; userPicture = ''; userEmail = '';
